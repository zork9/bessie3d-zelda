...
*/
int PixmapTexture::drawmain(SDL_Surface *screen, int offsetx, int offsety)
{
	...
	drawbiggerandrotate(screen,offsetx+100,offsety+100);
	...
	for (int yy = 0; yy < _enlargeh; yy++) {
		for (int xx = 0; xx < _enlargew; xx++) { 
			...
			int x1 = offsetx+(int)pt.get_x();
			int y1 = offsety+(int)pt.get_y();
			int x2 = offsetx+(int)pt.get_x();
			int y2 = offsety+(int)pt.get_y();
			drawline(screen,&x1,&y1,&x2,&y2,0xff);
			for (int k = prevy/_beta; k < pt.get_y(); k++)  {
				for (int j = prevx/_alpha; j < pt.get_x(); j++)  {
				int x1 = offsetx+(int)prevx/_alpha+j;
				int y1 = offsety+(int)prevy/_beta+k;
				int x2 = offsetx+(int)prevx/_alpha+j;
				int y2 = offsety+(int)prevy/_beta+k;
				drawline(screen,&x1,&y1,&x2,&y2,0xff);
			}
			}
...
int PixmapTexture::drawbiggerandrotate(SDL_Surface *screen,int offsetx, int offsety)
{
	...
	for (int yy = 0; yy < _h*_beta; yy ++) {
		for (int xx = 0; xx < _w*_alpha; xx ++) { 
			...
			//Point pt = p2.scalex(angle);//_pixmaptexture.get_alpha());
			//unsigned long color = 65000;
			unsigned long color = 255;//get_xpmimage().getpixel((int)pt.get_x(), (int)pt.get_y());
			...
			//prot = rotmy*prot;
			prot = rotmx*prot;
			int x1 = offsetx+(int)xx;
			int y1 = offsety+(int)yy;
			int x2 = offsetx+(int)xx;
			int y2 = offsety+(int)yy;
			if (color != 0)//alpha channel black
			drawline(screen,
				&x1,&y1,&x2,&y2,0xff);
			/*XDrawLine(*dpy, *w, gc,
